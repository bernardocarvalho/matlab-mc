	...
	for ele=1:1:length(elmts)%correr os elementos selcionados
		...
		for gNdCol=1:1:Nnodes % %correr as colunas da matriz K
			...
			if length(iNd)==1
				iNdCol=iNd;
				%rangeK
				K((2*gNdRow-1):(2*gNdRow),(2*gNdCol-1):(2*gNdCol)) =K((2*gNdRow-1):(2*gNdRow),(2*gNdCol-1):(2*gNdCol)) +  KELE{ele}((2*iNdRow-1):(2*iNdRow),(2*iNdCol-1):(2*iNdCol)) ;     % Add entries at KGlobal
			end
		end
	end
